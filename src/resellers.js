$(document).ready(function () {
  function createBadgeImages(partnerStatus, partnerBadges) {
    ...
    if (partnerStatus) {
      badgesHtml.push(
        `<img class="reseller_box-badge" src="${createImageUrl(
          "status",
          partnerStatus
        )}" alt="${partnerStatus}">`
      );
  ...
  function updateResellerItem($item, partner) {
    ...
    $item
      .find('[data-element="reseller-logo"]')
      .attr("src", partner.logo || "");
    $item.find('[data-element="reseller-name"]').text(partner.name || "");
    $item.find('[data-element="reseller-city"]').text(partner.city || "");

    const $categoriesWrapper = $item.find(
      '[data-element="reseller-categories"]'
    );
    $categoriesWrapper.empty();

    const tickSvg =
      '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.0001 10.7799L3.2201 7.9999L2.27344 8.9399L6.0001 12.6666L14.0001 4.66656L13.0601 3.72656L6.0001 10.7799Z" fill="#03081C"/></svg>';
    ...
    // Handle badges
    const badgesHtml = createBadgeImages(
      partner.partner_status,
      partner.partner_badges
    );

    const $badgesWrapper = $item.find(
      '[data-element="reseller-badges-wrapper"]'
    );
    const $badgesSection = $item.find('[data-element="badges-section"]');

    if (
      !partner.partner_status &&
      (!partner.partner_badges || partner.partner_badges.length === 0)
    ) {
      // If no badges and no status, remove the badges section
      if ($badgesSection.length) {
        $badgesSection.remove();
      }
    } else {
      // If there are badges or status, update the content
      const badgesHtml = createBadgeImages(
        partner.partner_status,
        partner.partner_badges
      );
  ...
  function updateTags() {
    ...
    $(
      '[data-element="list-zakres"], [data-element="list-specialization"], [data-element="list-status"]'
    )
      .find("input:checked")
      .each(function () {
  ...
  function filterResellers() {
    const selectedCategories = $('[data-element="list-zakres"] input:checked')
      .map(function () {
    ...
    const selectedBadges = $(
      '[data-element="list-specialization"] input:checked'
    )
      .map(function () {
        return $(this).next("label").text();
      })
    ...
    const selectedStatuses = $('[data-element="list-status"] input:checked')
      .map(function () {
        return $(this).next("label").text();
      })
      .get();
    ...
    $('[data-element="reseller-item"]').each(function () {
      ...
      const categories = $item
        .find(
          '[data-element="reseller-categories"] .category-item span:last-child'
        )
        .map(function () {
          return $(this).text();
        })
      ...
      const badges = $item
        .find('[data-element="reseller-badges-wrapper"] img')
        .map(function () {
      ...
      const name = $item
        .find('[data-element="reseller-name"]')
        .text()
        .toLowerCase();
      ...
      const matchesBadges =
        ...
        selectedBadges.some((badge) => badges.includes(badge));
      const matchesStatus =
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        const $template = $wrapper
          .children('[data-element="reseller-item"]')
          .first();
        ...
        if (
          ...
          populateFilterList(
            '[data-element="list-zakres"]',
            allCategories,
            "category"
          );
          populateFilterList(
            '[data-element="list-specialization"]',
            allBadges,
            "badge"
          );
          populateFilterList(
            '[data-element="list-status"]',
            allStatuses,
            "status"
          );
