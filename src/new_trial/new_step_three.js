...
$(document).ready(function() {
    ...
    function toggleClientTypeFields() {
        ...
        $companyWrapper.toggleClass('hide', !isCompany).find('input').prop('disabled', !isCompany).attr('data-exclude', isCompany ? null : 'true');
        $consumentWrapper.toggleClass('hide', isCompany).find('input').prop('disabled', isCompany).attr('data-exclude', isCompany ? 'true' : null);
        clearAllErrors();
    }
...
$(document).on('formSubmissionComplete', function(event, isSuccess, $form, $field, data) {
    if (isSuccess) {
        DataLayerGatherers.pushEmailSubmittedData(window.myGlobals.clientId, window.myGlobals.shopId, $form.data('action'), "********");
    } else {
        DataLayerGatherers.pushTrackEventError($form.data('action'), $form.find("#label").text(), "********");
    }
});
