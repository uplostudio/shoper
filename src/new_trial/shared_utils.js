...
const $twoStepTrialsWrapper = $("[data-element='trials-wrapper-two-steps']");
...
// Common functions
SharedUtils.enforceDisplay = function($element, displayValue) {
    const observer = new MutationObserver((mutations)=>{
        mutations.forEach((mutation)=>{
            if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
                if ($element.css('display') !== displayValue) {
...
SharedUtils.handleResponse = function(data, $form, $field, $wFormFail, isSuccess, stepNumber) {
    if (isSuccess) {
        this.handleSuccessResponse(data, $form, $field, $wFormFail, stepNumber);
    } else {
        this.handleErrorResponse(data, $form, $field, $wFormFail, stepNumber);
    }

    $(document).trigger('formSubmissionComplete', [isSuccess, $form, $field, data]);
}
;
...
SharedUtils.handleSuccessResponse = function(data, $form, $field, $wFormFail, stepNumber) {
    ...
    if (data.host) {
        window.myGlobals.host = data.host;
    }
    if (data.shop_id) {
        window.myGlobals.shopId = data.shop_id;
    }
    if (data.license_id) {
...
SharedUtils.checkAndUpdateSID = function() {
    ...
    if (hoursSinceStored < 24) {
        ...
        $('[data-formid="create_trial_step1_new"], [data-formid="create_trial_step2"], [data-formid="create_trial_step2_new"]').attr('data-sid', parsedSidData.value);
    } else {
        localStorage.removeItem('sid');
        $('[data-formid="create_trial_step1_new"], [data-formid="create_trial_step2"], [data-formid="create_trial_step2_new"]').removeAttr('data-sid');
        this.setCurrentSID(null);
    }
}
;
...
SharedUtils.setCurrentSID = function(sid) {
    currentSID = sid;
}
;

SharedUtils.populateCountrySelect = function(selectElement, selectedCountry="PL") {
    $.each(this.countriesList, function(index, country) {
        let option = $("<option></option>").text(country.name_pl).val(country.code);
