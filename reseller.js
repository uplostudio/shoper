...
$(document).ready(function() {
    const updateElements = (clickId, unclickId, enableElem, disableElem) => {
        $('#' + clickId).removeAttr('data-exclude');
        $('#' + unclickId).attr('data-exclude', 'true');
        enableElem.removeClass('hide').find('input').prop('disabled', false).removeAttr('data-exclude');
        disableElem.addClass('hide').find('input').prop('disabled', true).attr('data-exclude', 'true');
    }
    ...
    $('#1').click();
    $('#0').attr('data-exclude', 'true');
    $consumentWrapper.find('input').prop('disabled', true).attr('data-exclude', 'true');

    $("#wf-form-reseller input[type='radio']").change(function() {
        const currentRadioInputValue = this.value;

        $("#wf-form-reseller label").removeClass("is-checked");

        if ($(this).prev().hasClass("w--redirected-focus")) {
            $(this).closest("label").addClass("is-checked");
        }

        if (currentRadioInputValue === '1') {
            updateElements('1', '0', $companyWrapper, $consumentWrapper);
        } else {
            updateElements('0', '1', $consumentWrapper, $companyWrapper);
        }
    });
});
